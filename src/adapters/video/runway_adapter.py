"""
VJ-Gen Runway 视频生成适配器
调用 Runway Gen-3 API 生成视频
"""
...
class RunwayAdapter(VideoGenAdapter):
    """Runway Gen-3 适配器"""

    def __init__(self, config: AdapterConfig):
        super().__init__(config)
        self._base_url = config.api_base or "https://api.dev.pika.run/v1"
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                response = await client.post(
                    f"{self._base_url}/generation/create",
                    headers={
    ...
    async def generate_from_text(
        ...
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                response = await client.post(
                    f"{self._base_url}/generation/text-create",
                    headers={
                        "Authorization": f"Bearer {self.config.api_key}",
                        "Content-Type": "application/json",
                    },
    ...
    async def extend(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.config.timeout) as client:
                response = await client.post(
                    f"{self._base_url}/generation/extend",
                    headers={
                        "Authorization": f"Bearer {self.config.api_key}",
                        "Content-Type": "application/json",
                    },
    ...
    def get_capabilities(self) -> list["AdapterCapability"]:
        ...
        return [
            AdapterCapability(
                ...
                description="Generate video from keyframe image",
                supported_params=["keyframe_path", "prompt", "duration", "resolution"],
                limitations=["Max 10 seconds, 720p resolution"],
    ...
    async def _poll_until_complete(self, task_id: str, start_time: float) -> VideoResult | None:
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout) as client:
            while (time.time() - start_time) < max_wait:
                response = await client.get(
                    f"{self._base_url}/generation/{task_id}",
                    headers={"Authorization": f"Bearer {self.config.api_key}"},
                )
                    ...
                    if video_url:
                        ...
                        if video_path:
                            return VideoResult(
                                ...
                                thumbnail_path=result.get("output", {}).get("thumbnail_url", ""),
